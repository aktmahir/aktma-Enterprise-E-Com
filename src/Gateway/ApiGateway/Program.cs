...
app.MapGet("/api/v1/catalog/products", async (IHttpClientFactory clients, IConfiguration configuration, string? search, string? category, int? page, int? pageSize, CancellationToken cancellationToken) => { var baseUrl = configuration["Services:Catalog"] ?? "http://localhost:5013"; var query = new QueryString().Add("search", search).Add("category", category).Add("page", page?.ToString()).Add("pageSize", pageSize?.ToString()); var client = clients.CreateClient(); return Results.Content(await client.GetStringAsync($"{baseUrl}/api/v1/products{query}", cancellationToken), "application/json"); });
...
{
	var baseUrl = configuration["Services:Orders"] ?? "http://localhost:5041";
